...
class DiffServiceAdapterTest {
  ...
  void computeDifference_resultMapped() {
    doReturn(DiffResult.builder().outcome(DiffResultOutcome.DIFFERENT_WITH_OFFSETS).offsets(List.of(
        DiffResultOffset.builder().offset(0).length(2).build(), DiffResultOffset.builder().offset(4).length(3).build()
    )).build()).when(diffDomainService).computeDifference(1L);

    DiffResultDto diffResultDto = diffServiceAdapter.computeDifference(1L);

    verify(diffDomainService).computeDifference(1L);
    assertThat(diffResultDto).isEqualTo(DiffResultDto.builder().outcome("DIFFERENT_WITH_OFFSETS").offsets(List.of(
        DiffResultOffsetDto.builder().offset(0).length(2).build(), DiffResultOffsetDto.builder().offset(4).length(3).build()
    )).build());
  }
}
